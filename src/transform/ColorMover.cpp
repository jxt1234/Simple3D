...
void CColorMover::Inti(double kMin,double kMax,double ColorVMin,double ColorVMax, int* randList)
{
    m_kR=randList[0]*(1.0/RAND_MAX)*(kMax-kMin)+kMin; 
    m_kG=randList[1]*(1.0/RAND_MAX)*(kMax-kMin)+kMin; 
    m_kB=randList[2]*(1.0/RAND_MAX)*(kMax-kMin)+kMin;  

    m_VR=randList[3]*(1.0/RAND_MAX)*(ColorVMax-ColorVMin)+ColorVMin;
    m_VG=randList[4]*(1.0/RAND_MAX)*(ColorVMax-ColorVMin)+ColorVMin;
    m_VB=randList[5]*(1.0/RAND_MAX)*(ColorVMax-ColorVMin)+ColorVMin;
    if (randList[6]>RAND_MAX/2) m_VR*=(-1);
    if (randList[7]>RAND_MAX/2) m_VG*=(-1);
    if (randList[8]>RAND_MAX/2) m_VB*=(-1);

    m_R0=randList[9]*(1.0/RAND_MAX)*csMaxColorRoundValue;
    m_G0=randList[10]*(1.0/RAND_MAX)*csMaxColorRoundValue;
    m_B0=randList[11]*(1.0/RAND_MAX)*csMaxColorRoundValue;
}
